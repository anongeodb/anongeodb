 ...
 */
public class GeoDBStatement implements OBDAStatement {
	...
	private static final Logger log = LoggerFactory.getLogger(GeoDBStatement.class);
	...
	public GeoDBStatement(Quest questinstance, QuestConnection conn, Statement st, NodeSelectivityEstimator nse) {
	...
	public String getSPARQLRewriting(String sparql) throws OBDAException {
		...
		System.out.println("GeoDBStatement.translateAndPreprocessed:" + initialProgram.toString());
	...
	public SQLResult getUnfolding(String strquery, boolean splitSpatialJoin) throws Exception {
		...
		if (questInstance.hasCachedSQL(strquery)) {
			...
			System.out.println("GeoDBStatement: parsed query" + query);
	...
	private boolean queryHasSpatialJoin(CQIE initial, Set<Variable> varsInSpatial, List<Function> spatialJoins) {
		for (Function atom : initial.getBody()) {
			...
			if (GeoDBParameters.isSpatialFunction(atom)) {
	...
	public void cancel() throws OBDAException {
		canceled = true;
		try {
			GeoDBStatement.this.executionthread.cancel();
		} catch (Exception e) {
			throw new OBDAException(e);
		}
	}
